...
/// Defines a quadratic extension of the cubic extension field using an irreducible polynomial x² + 1.
...
// Implementing the extension Fp^6 as Fp^3[x] / (x^2 + 1).
    ...
    Self: FieldElement<BaseField = B>,
{
    ...
    fn mul(a: [Self; 2], b: [Self; 2]) -> [Self; 2] {
        let a0b0 = a[0] * b[0];
        let a1b1 = a[1] * b[1];
        [a0b0 - a1b1, (a[0] + a[1]) * (b[0] + b[1]) - a0b0 - a1b1]
    }
    ...
    fn frobenius(x: [Self; 2]) -> [Self; 2] {
        // given x = α + β * φ
        // frobenius(x) = froenius(α) + frobenius(β) * frobenius(φ)
        //              = froenius(α) + frobenius(β) * φ
