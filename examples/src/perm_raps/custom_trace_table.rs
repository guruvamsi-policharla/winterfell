...
impl<B: StarkField> Trace for RapTraceTable<B> {
        ...
        E: FieldElement<BaseField = Self::BaseField>,
    {
        ...
        aux_columns[0][0] = E::ZERO;
        aux_columns[1][0] = E::ZERO;
        aux_columns[2][0] = E::ZERO;
        
        // aux_columns[0][0] = E::ONE*current_row[0].into();
        // aux_columns[1][0] = E::ONE*current_row[1].into();
        // aux_columns[2][0] = E::ONE*current_row[2].into() + rand_elements[0].exp(E::PositiveInteger::from(n as u32))*current_row[3].into();

        for index in 0..self.length()-1 {
            self.read_row_into(index, &mut current_row);

            // TODO: AVOID EXPONENTIATION BY REMEMBERING PREVIOUS ITERATION VALUE
            let gamma1 = rand_elements[0].exp(E::PositiveInteger::from(index as u32));
            let gamma2 = rand_elements[0].exp(E::PositiveInteger::from((index+n-1) as u32));
            // println!("index: {}, index+n: {}", index, index+n-1);

            aux_columns[0][index+1] = aux_columns[0][index] + gamma1*current_row[0].into();
            aux_columns[1][index+1] = aux_columns[1][index] + gamma1*current_row[1].into();
            aux_columns[2][index+1] = aux_columns[2][index] + gamma1*current_row[2].into() + gamma2*current_row[3].into();
            aux_columns[3][index] = gamma1;
            aux_columns[4][index] = gamma2;
            // Likely need to fix this
        }

        aux_columns[3][self.length()-1] = aux_columns[3][self.length()-2]*rand_elements[0];
        aux_columns[4][self.length()-1] = aux_columns[4][self.length()-2]*rand_elements[0];
